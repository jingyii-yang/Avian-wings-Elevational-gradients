...
# calculate data distribution of HWI and HWA of each band
...
elev$all_ranges_max = ifelse(is.na(elev$Updated_max.elevation), elev$Ref.1_max.elevation, elev$Updated_max.elevation)
elev$all_ranges_min = ifelse(is.na(elev$Updated_min.elevation), elev$Ref.1_min.elevation, elev$Updated_min.elevation)
elev$all_ranges_mean = (elev$all_ranges_max + elev$all_ranges_min)/2
# get elevation data that consider breeding ranges only
elev$breeding_range_mean = (elev$Ref.2_max.elevation + elev$Ref.2_min.elevation)/2

# fig. S5A (max elevation)
elev_plot = na.omit(elev[, c('all_ranges_max', 'Ref.2_max.elevation', 'Migration')])
elev_plot$Migration = ifelse(elev_plot$Migration == 3, 'Yes', 'No')
cor2 = cor.test(elev_plot$all_ranges_max, elev_plot$Ref.2_max.elevation)
pa=ggplot(elev_plot, aes(all_ranges_max/1000, Ref.2_max.elevation/1000, col=Migration)) + theme_classic()+
      geom_point(size = 1, alpha = 0.5) +
...
elev_plot = na.omit(elev[, c('all_ranges_mean', 'breeding_range_mean', 'Migration')])
elev_plot$Migration = ifelse(elev_plot$Migration == 3, 'Yes', 'No')
cor3 = cor.test(elev_plot$all_ranges_mean, elev_plot$breeding_range_mean)
pb=ggplot(elev_plot, aes(all_ranges_mean/1000, breeding_range_mean/1000, col=Migration)) + theme_classic()+
      geom_point(size = 1, alpha = 0.5) +
      theme(axis.title = element_text(size=20),
            axis.text = element_text(size=18), legend.position = 'none') +
      labs(x='Elevation - all ranges (km)', y = 'Elevation - breeding range (km)') +
